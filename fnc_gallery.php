<?php
	...
	function count_public_photos($privacy){
		...
		$conn = new mysqli($GLOBALS["server_host"], $GLOBALS["server_user_name"], $GLOBALS["server_password"], $GLOBALS["database"]);
		$conn->set_charset("utf8");
		$stmt = $conn->prepare("SELECT COUNT(filename) FROM vpr_photos WHERE privacy >= ? and deleted IS NULL");
		echo $conn->error;
		$stmt->bind_param("i", $privacy);
	...
	function count_own_photos(){
		...
		$conn = new mysqli($GLOBALS["server_host"], $GLOBALS["server_user_name"], $GLOBALS["server_password"], $GLOBALS["database"]);
		$conn->set_charset("utf8");
		$stmt = $conn->prepare("SELECT COUNT(filename) FROM vpr_photos WHERE userid = ? and deleted IS NULL");
		echo $conn->error;
	...
	function read_own_photo_thumbs($page, $limit){
		...
		while($stmt->fetch()){
			...
			//</a>
			//</div>
			//UPDATE vpr_photos SET deleted = NOW() WHERE id = ?
			$photo_html .= '<div class="thumbgallery">' ."\n";
